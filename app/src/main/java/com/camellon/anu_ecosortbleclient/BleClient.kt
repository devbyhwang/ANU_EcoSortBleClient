    ...
    fun connect(device: BluetoothDevice) {
        ...
        val callback = object : BluetoothGattCallback() {
            override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
                ...
                when (newState) {
                    BluetoothGatt.STATE_CONNECTED -> {
                        if (!gatt.discoverServices()) failAndClose(gatt, "discoverServices() returned false")
                    }
            ...
            override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
                ...
                val characteristic: BluetoothGattCharacteristic = service.getCharacteristic(notifyUuid) ?: run {
                    failAndClose(gatt, "Notify char UUID not found: $notifyUuid")
                    return
                }
                ...
                if (!gatt.writeDescriptor(descriptor)) failAndClose(gatt, "writeDescriptor() returned false")
            }
    ...
    private fun handlePayload(raw: String) {
        ...
        runCatching {
            ...
            try {
                ...
                Log.d("BleClient", "JSON 아님. 일반 텍스트 처리: $raw")
            }
            ...
            Log.e("BleClient", "데이터 처리 에러: $raw", it)
        }
    }
